...
@testset "DEBUGGING: Step-by-Step Inference Analysis" begin
    ...
    @testset "STEP 1: Device Selection and GPU Health" begin
        ...
        try
            test_arr = oneArray(Float32[1.0, 2.0, 3.0])
    ...
    @testset "STEP 5: Tokenizer Loading and Validation" begin
        ...
        @test tok.eos_id > 0
        @test tok.bos_id ≥ 0  # BOS might be 0 for some models
    ...
    @testset "STEP 8: Forward Pass - Prefill" begin
        ...
        try
            ...
            @test logits_max < 1000.0f0 "Logits too large"
            @test logits_min > -1000.0f0 "Logits too small"
    ...
    @testset "STEP 10: Sampling and Token Generation" begin
        ...
        test_logits = [
            Float32[1.0, 5.0, 2.0, 4.0, 3.0],  # Clear max at index 2
            Float32[-1.0, -5.0, -2.0, -4.0],  # Clear max at index 1 (least negative)
            Float32[0.1, 0.1, 0.1, 0.1],      # All equal
        ]
        ...
        for (i, logits) in enumerate(test_logits)
            ...
            greedy_result = Inferno.Engine.sample(logits, 0.0f0, 1.0f0)
            ...
            temp_result = Inferno.Engine.sample(logits, 0.7f0, 0.9f0)
        ...
        strategies = [
            ("greedy", 0.0f0, 1.0f0),
            ("low_temp", 0.3f0, 0.9f0),
            ("med_temp", 0.7f0, 0.8f0),
            ("high_temp", 1.0f0, 0.9f0),
        ]
    ...
    @testset "STEP 11: End-to-End Generation Validation" begin
        ...
        for prompt in test_prompts
            ...
            try
                ...
                # Generate tokens
                for step in 1:5
                    token_id = Inferno.Engine.sample(last_logits, 0.7f0, 0.8f0)
                    token_str = Inferno.Tokenizer.decode(tok, [token_id])
