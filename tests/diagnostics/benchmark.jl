...
function init_kv_cache_baseline(head_dim, n_kv, max_seq)
    ...
    k = oneArray(zeros(Float16, head_dim, n_kv, seq))
    v = oneArray(zeros(Float16, head_dim, n_kv, seq))
    oneAPI.synchronize()
    return k, v
end
...
function init_kv_cache_optimized(head_dim, n_kv, max_seq)
    seq = min(max_seq, 512)
    k = oneArray{Float16}(undef, head_dim, n_kv, seq)
    v = oneArray{Float16}(undef, head_dim, n_kv, seq)
    fill!(k, 0.0f0)
    fill!(v, 0.0f0)
    oneAPI.synchronize()
    return k, v
