    ...
    @testset "GPU Device Health" begin
        ...
        for (i, dev) in enumerate(devs)
            try
                ...
                test_arr = oneArray(Float16[Float16(1.0), Float16(2.0), Float16(3.0)])
                test_sum = sum(test_arr)
                @test test_sum ≈ Float16(6.0)

                # Test larger allocation (1MB)
                large_arr = oneArray(zeros(Float16, 256 * 1024))
                large_sum = sum(large_arr)
                @test large_sum == Float16(0.0)
    ...
    @testset "Forward Pass Robustness" begin
        ...
        for (case_name, prompt) in test_cases
            ...
            try
                ...
                @test max_logit < Float16(1000.0)
                @test min_logit > -Float16(1000.0)
    ...
    @testset "Generation Stability" begin
        ...
        for prompt in test_prompts
            ...
            try
                ...
                for step in 1:3
                    # Sample token
                    token_id = Inferno.Engine.sample(current_logits, Float16(0.7), Float16(0.8))
