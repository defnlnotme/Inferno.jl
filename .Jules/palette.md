...
## 2025-05-16 - [Informative Progress Indicators]
**Learning:** For long-running batch operations like model weight loading, fractional progress indicators (`n/total`) are superior to indeterminate indicators (like dots) as they provide clear expectation of remaining time. Using ANSI escape codes like `\r` (carriage return) and `\e[K` (or just overwriting) to update the same line keeps the terminal clean and prevents scroll-bloat.
**Action:** Prefer fractional progress (`$i/$total`) with `\r` for known-length loops over simple dots to improve user feedback quality.
