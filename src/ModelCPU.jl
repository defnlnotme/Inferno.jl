...
function lm_head_project!(output::Vector{Float32}, weight::Matrix{Float32}, hidden::Vector{Float32}; nchunks::Int=8)
 ...
 # Use @sync + @spawn for parallel projection without redundant allocations
 @sync for chunk in 1:nchunks
 i_start = (chunk - 1) * chunk_size + 1
 i_end = min(chunk * chunk_size, vocab_size)
 
 Threads.@spawn begin
 # Write directly to output view, eliminating redundant allocations and copy loops
...
function lm_head_project!(output::Vector{Float32}, weight::Matrix{BFloat16}, hidden::Vector{Float32}; nchunks::Int=8)
 ...
 # We use ArrowLake.bf16_gemv_c_cols! to compute dot products for each column
 # and write directly to the output buffer view to avoid allocations.
 ...
 buf, W_u16, x_bf16, j_start, j_end, size(weight, 1))
 end
 end
end
