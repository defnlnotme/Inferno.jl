...
Allocations: Zero intermediate buffer allocations (~600KB saved per token).
"""
function lm_head_project!(output::Vector{Float32}, weight::Matrix{Float32}, hidden::Vector{Float32}; nchunks::Int=8)
    ...
    # Use @sync to wait for all spawned tasks, writing directly to output views
    # to eliminate intermediate allocations (~600KB/token) and redundant copies.
...
function lm_head_project!(output::Vector{Float32}, weight::Matrix{BFloat16}, hidden::Vector{Float32}; nchunks::Int=8)
    ...
    # Use @sync to wait for all spawned tasks, writing directly to output views
    # to eliminate intermediate allocations and redundant copies.
    @sync for chunk in 1:nchunks
        ...
        Threads.@spawn begin
            # Pass a view of the final output vector directly to the C kernel
