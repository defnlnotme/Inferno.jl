...
function lm_head_project!(output::Vector{Float32}, weight::Matrix{Float32}, hidden::Vector{Float32}; nchunks::Int=8)
    ...
    # Optimized Bolt: Use @sync and Threads.@spawn to parallelize without temporary allocations
    # Writing directly to output views eliminates ~600KB allocation and redundant copy loop per token.
    # default nchunks=8 is optimal for modern CPUs (as per AGENTS.md).
...
function lm_head_project!(output::Vector{Float32}, weight::Matrix{BFloat16}, hidden::Vector{Float32}; nchunks::Int=8)
    ...
    @sync for chunk in 1:nchunks
        ...
        Threads.@spawn begin
            # Use the GEMV kernel for columns: treat each column as a "row" for the dot product
            # Pass a view of contiguous columns to the C kernel
            out_view = view(output, j_start:j_end)
            ArrowLake.bf16_gemv_c_cols!(out_view, W_u16, x_bf16, j_start, j_end, hidden_size)
        end
    end
    return output
end
