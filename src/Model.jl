...
function softmax_kernel!(probs, scores, len, scale)
    # Perform softmax entirely on GPU using vectorized broadcasting and reductions
    ps = @view probs[1:len, 1]
    ss = @view scores[1:len, 1]

    # Max reduction on GPU
    mx = Float32(maximum(ss) * scale)

    # Compute exponentials with Float32 stability
    @. ps = exp(Float32(ss * scale) - mx)

    # Sum reduction on GPU using Float32 for precision
    s = mapreduce(v -> Float32(v), +, ps)
...
function batched_softmax_kernel!(probs, scores, total_len, n_heads, scale)
    # Batched softmax entirely on GPU; vectorizes across heads
    ps = @view probs[1:total_len, 1:n_heads]
    ss = @view scores[1:total_len, 1:n_heads]

    # Max reduction per head (dim=1)
    mx = maximum(ss, dims=1) .* Float32(scale)

    # Compute exponentials with Float32 stability
    @. ps = exp(Float32(ss * scale) - mx)

    # Sum reduction per head using Float32 for precision
    s = mapreduce(v -> Float32(v), +, ps, dims=1)
...
function (m::FullAttention)(x::oneArray{Float16,2}, pos::Int, rope::RotaryEmbedding, cache::KVCache)
 ...
 # 4. Gate (applied AFTER attention, not to Q)
        # Compute sigmoid gating entirely on GPU using Float32 stability
