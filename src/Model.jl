...
function (m::MoE)(x::oneMatrix{Float32})
    ...
    output = oneArray{Float32}(undef, hidden_size, seq_len); fill!(output, 0.0f0)
...
function FullAttention(arch, wq, wk, wv, wqkv, wo, q_norm, k_norm, n_heads, n_kv, hd)
    ...
    # Optimized: direct GPU allocation with undef and fill! to avoid CPU zeroing and transfer
    decode_q_full = oneArray{Float32}(undef, q_size, 1); fill!(decode_q_full, 0.0f0)
    decode_k = oneArray{Float32}(undef, hd * n_kv, 1); fill!(decode_k, 0.0f0)
    decode_v = oneArray{Float32}(undef, hd * n_kv, 1); fill!(decode_v, 0.0f0)
    decode_combined = oneArray{Float32}(undef, hd * n_heads, 1); fill!(decode_combined, 0.0f0)
    
    # Fixed buffer sizes - sufficient for 4096 context
    max_len = 4096
    decode_scores = oneArray{Float32}(undef, max_len, 1); fill!(decode_scores, 0.0f0)
    decode_pb = oneArray{Float32}(undef, max_len, 1); fill!(decode_pb, 0.0f0)
    decode_out_h = oneArray{Float32}(undef, hd, 1); fill!(decode_out_h, 0.0f0)
    wo_out_size = size(wo, 1)
    decode_wo_buf = oneArray{Float32}(undef, wo_out_size, 1); fill!(decode_wo_buf, 0.0f0)
    oneAPI.synchronize()
    
    # Prefill buffers - fixed size for 4096 context
    prefill_scores = oneArray{Float32}(undef, max_len, n_heads); fill!(prefill_scores, 0.0f0)
    prefill_pb = oneArray{Float32}(undef, max_len, n_heads); fill!(prefill_pb, 0.0f0)
...
function (m::FullAttention)(x::oneArray{Float32,2}, pos::Int, rope::RotaryEmbedding, cache::KVCache)
    ...
    if seq == 1
        ...
        kv_per_q = m.n_heads ÷ m.n_kv
        # Optimized: direct GPU allocation with undef and fill!
        combined_all = oneArray{Float32}(undef, size(q_gated, 1) * m.n_heads, seq); fill!(combined_all, 0.0f0)
...
function (m::MLAttention)(x::oneMatrix{Float32}, pos::Int, rope::RotaryEmbedding, cache::KVCache)
    ...
    # Optimized: direct GPU allocation with undef and fill!
    out = oneArray{Float32}(undef, size(x, 1), seq); fill!(out, 0.0f0); return out
end
...
function GatedDeltaNet(in_proj, gate_proj, ssm_out, ssm_a, ssm_alpha, ssm_beta, ssm_conv1d, ssm_conv1d_cpu, ssm_dt_bias, ssm_norm,
    ...
    # Optimized: direct GPU allocation with undef and fill!
    decode_beta = oneArray{Float32}(undef, num_v_heads, 1); fill!(decode_beta, 0.0f0)
    decode_alpha = oneArray{Float32}(undef, num_v_heads, 1); fill!(decode_alpha, 0.0f0)
    decode_decay_gate = oneArray{Float32}(undef, num_v_heads, 1); fill!(decode_decay_gate, 0.0f0)
    decode_conv_out = oneArray{Float32}(undef, conv_channels, 1); fill!(decode_conv_out, 0.0f0)
    decode_z = oneArray{Float32}(undef, d_inner, 1); fill!(decode_z, 0.0f0)
    decode_output_normed = oneArray{Float32}(undef, head_v_dim * num_v_heads, 1); fill!(decode_output_normed, 0.0f0)
    decode_gated = oneArray{Float32}(undef, d_inner, 1); fill!(decode_gated, 0.0f0)
    
    # Prefill buffers - sized for max context
    max_seq = 4096
    prefill_beta = oneArray{Float32}(undef, num_v_heads, max_seq); fill!(prefill_beta, 0.0f0)
    prefill_alpha = oneArray{Float32}(undef, num_v_heads, max_seq); fill!(prefill_alpha, 0.0f0)
    oneAPI.synchronize()
