...
function softmax_kernel!(probs, scores, len, scale)
    # Optimized: Keep computation on GPU to avoid CPU-GPU round-trip
    sc_view = view(scores, 1:len, 1)
    pr_view = view(probs, 1:len, 1)

    # Compute max in Float32 for stability
    mx = mapreduce(v -> Float32(v), max, sc_view) * Float32(scale)

    # exp(x - mx) using GPU broadcasting
    @. pr_view = Float16(exp(Float32(sc_view * scale) - mx))
...
function batched_softmax_kernel!(probs, scores, total_len, n_heads, scale)
    ...
    # Compute max for each head on GPU (result is 1 x n_heads)
    mx = mapreduce(v -> Float32(v), max, sc_view, dims=1) .* Float32(scale)

    # exp(x - mx) using GPU broadcasting
    @. pr_view = Float16(exp(Float32(sc_view * scale) - mx))

    # Sum each head on GPU
...
function (m::MLP)(x::oneMatrix{Float16}, cache::KVCache)
    # Optimized: Use GPU broadcasting for SiLU to avoid CPU-GPU round-trip
    mul!(cache.mlp_gate, m.gate_weight, x)
    mul!(cache.mlp_up, m.up_weight, x)

    # Numerically stable SiLU on GPU: gate * sigmoid(gate)
    # Use Float32 intermediate during broadcast to avoid Float16 exp() overflow
...
function (m::MoE)(x::oneMatrix{Float16}, cache::KVCache)
    ...
    for t in 1:seq_len
        ...
        for (i, expert_idx) in enumerate(top_k_indices)
            # Optimized: Keep SiLU computation on GPU
...
function (m::FullAttention)(x::oneArray{Float16,2}, pos::Int, rope::RotaryEmbedding, cache::KVCache)
    ...
    if m.architecture == :qwen || m.architecture == :qwen2 || m.architecture == :qwen2_5 || m.architecture == :qwen35
        ...
        # Optimized: Keep sigmoid computation on GPU to avoid CPU-GPU round-trip
        # Use Float32 intermediate during broadcast to avoid Float16 exp() overflow
