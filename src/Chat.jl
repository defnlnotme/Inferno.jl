...
function stream_with_colors(model, tok, prompt; io::IO=stdout, stop_tokens::Set{Int}=Set{Int}(), max_tokens::Int=512, thinking_enabled::Bool=false, show_tps::Bool=false, kwargs...)
    ...
    for token in generate_stream_cpu(model, prompt_tokens, (ids) -> decode(tok, ids); 
        ...
        if first_token
            if is_stdout_tty
                # Clear thinking indicator: move back 3 spaces and clear to end of line
                print(io, "\b\b\b\e[K")
            end
            first_token = false
        end
...
function display_help(term)
    printstyled(term, "Available commands:\n", bold=true)
    ...
    printstyled(term, "  /quit", color=:yellow); print(term, ", "); printstyled(term, "/exit", color=:yellow); println(term, " - Exit chat")
end
